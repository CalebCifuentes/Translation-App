...
async function processAudio(ws, data, runPipeline) {
    ...
    if (ws.session.isProcessing) {
        ws.send(JSON.stringify({ type: "error", message: "Audio already processing" }));
        return;
    }
    ...
    try {
        ...
        if (ws.session) {
            ws.send(JSON.stringify({
                ...
                audio: fs.statSync(result.output_file).size > 0 ? fs.readFileSync(result.output_file).toString("base64") : null
            }));
        }
    } catch (err) {
        console.error("Processing failed", err.message, err.stack);
        if (ws.session) {
            ws.send(JSON.stringify({ type: "error", message: "Audio Processing Failed" }));
        }
    } finally {
        if (ws.session) ws.session.isProcessing = false;
    }
}
